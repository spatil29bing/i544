...
function newOrder(app) {
  return (async function(req, res) {
    try {
      const query = locateQuery_Order(req.query);
     if (query.errors) {
        throw { errors: (query.errors ?? []) };
      }
      const {eateryId}  = query;
     const findEatery = await app.locals.dao.getEatery(eateryId);
      if(findEatery.errors) throw findEatery;
      const result =
        await app.locals.dao.newOrder(eateryId);
      result["items"] = []
      if (result.errors) throw result;
   const findEateryInfo = {...findEatery}
    const eateryInfo = getEateryInfo(findEateryInfo)
      const orderId = result.id;
     const links = [ selfLink(req) , eateryLink(req,eateryId) , orderLink(req,orderId) ]
     const final_result = {links, ...result,...eateryInfo,"total":0};
...
function getOrder(app) {
   return (async function(req, res) {
    try {
      ...
      if (result.errors) throw result;
      const eateryId = result.eateryId;
     const findEatery = await app.locals.dao.getEatery(eateryId);
      if(findEatery.errors) throw findEatery;
      const resultOne = await app.locals.dao.editOrder(orderId, 0, 0);
       if (resultOne.errors) throw resultOne;
     const ret= eateryOrder(findEatery, resultOne);
     const final_result = { links: [ selfLink(req),  eateryLink(req,eateryId) ], ...ret };
...
function updateOrder(app) {
    ...
    try {
      const orderId = req.params.orderId;
      const query = itemQuery(orderId,req.query);
      if (query.errors) {
        throw { errors: (query.errors ?? []) };
      }
       const {itemId,nItems } = query;
       const result = await app.locals.dao.getOrder(orderId);
      if (result.errors) throw result;
       const resultOne = await app.locals.dao.editOrder(orderId, itemId, nItems);
       if (resultOne.errors) throw resultOne;
	const eateryId = result.eateryId;
	const findEatery = await app.locals.dao.getEatery(eateryId);
        if(findEatery.errors) throw findEatery;
        const links= [ selfLink(req),  eateryLink(req,eateryId) ]
	const ret = eateryOrder(findEatery, resultOne);
        const final_Result = {links,...ret}
      res.json(final_Result);
    }
    catch(err) {
      //console.log(err); //uncomment during devel, especially for running tests
...
function getEateryInfo(findEateryInfo){
      const l = ["cuisine","id","loc","name"]
      const eateryInfo = {}
      for (const [k,v] of Object.entries(findEateryInfo)){
        for(const item of l){
		if(item === k){
			if(item === "id"){
			    eateryInfo["eateryId"] = v;
			}
			else{
			eateryInfo[k] = v;}
	}
...
function itemQuery(orderId,query) {
  const orderid = orderId
  const { itemId, nItems } = query
  const errors = [];
  if ((itemId ?? '').trim().length === 0 ) {
    errors.push(new AppError(`no itemId in update for order "${orderid}"`, {code: 'BAD_REQ'}));
  }
  else if ((nItems ?? '').trim().length === 0 ) {
    errors.push(new AppError(`bad nItems undefined in update for order "${orderid}"`,  {code: 'BAD_REQ'}));
  }
...
function locateQuery_Order(query){
  const { eateryId} = query
  const errors = [];
  if (eateryId === '') {
    errors.push(new AppError('missing eateryId parameter', {code: 'BAD_REQ'}));
  }
  return (errors.length > 0)
    ? { errors }
  : {eateryId};
...
function selfLink(req,orderId="0") {
  return {
    rel: 'self',
    name: 'self',
    href: (() => {let url_call = ''; if(orderId =="0") {url_call= selfUrl(req);}else{url_call=orderUrl(req, orderId);} return url_call;})(),
  };
}
